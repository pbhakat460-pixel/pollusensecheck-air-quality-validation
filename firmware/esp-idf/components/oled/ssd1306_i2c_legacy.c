...
/******************************************************************************
 * Initialize SSD1306 I2C Interface
 *
 * IMPORTANT:
 * The PolluSense project already manages the I2C controller through
 * i2c_bus.c.
 *
 * Therefore this function MUST NOT abort if the driver already exists.
 ******************************************************************************/
    ...
    int16_t reset)
{
    ...
    {
        ESP_LOGW(TAG,
                 "I2C driver already installed - reusing existing driver");
    }
    else
    {
        ESP_LOGE(TAG,
